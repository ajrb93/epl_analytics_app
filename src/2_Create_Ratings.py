...
def calculate_standings(fixtures):
    ...
    temp_home = fixtures[['season','home_team','away_team','GP','home_score','away_score','home_p',
    ...
    temp_away = fixtures[['season','home_team','away_team','GP','home_score','away_score','away_p',
...
def add_initial_season_ratings(team,team_ratings,season,team_initializations,transfer_vals):
    ...
    previous_season = str(int(temp_season.split('/')[0]) - 1) + '/' + str(int(temp_season.split('/')[1]) - 1)
...
def calculate_ratings(past_dates,transfer_vals,team_initializations,season_mapping,total_goals,home_field,results,update_rate):
    ...
    for date in past_dates:
        if date.month == 8 and date.day == 1:
            ...
            normalize_ratings(team_ratings, pd.to_datetime(season.split('/')[0]+'-08-01').date())
            normalize_ratings(team_ratings, pd.to_datetime(season.split('/')[0]+'-08-02').date())
        else:
...
team_ratings = calculate_ratings(define_dates_ratings(matches),transfer_vals,initializations,season_mapping,tg,hf,matches,2/38)
